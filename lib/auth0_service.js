...
class Auth0Service {
  ...
  _getApps(securityTrim, token) {
    securityTrim = securityTrim || function(apps) {
      return Promise.resolve(apps);
    };

    return this._requestApps(token)
      .then(apps => {
        var p = [];
  ...
  getApps(token) {
    return this._getApps(token);
  }

  getAppsForUser(user_id, token) {
    return this.getUserRoles(user_id)
      .then(user_roles => {
        ...
        return this._getApps(securityTrim, token);
      });
  }

  saveUser(user_id, user, token) {
    return new Promise((resolve, reject) => {
      request({
        method: 'PATCH',
        url: 'https://' + process.env.AUTH0_DOMAIN + '/api/v2/users/' + user_id,
  ...
  isAuthorized(user_id, client_id, token) {
    return this.getAppsForUser(user_id, token)
    .then(apps => {
      var i = _.findIndex(apps, { client_id: client_id });
      return i > -1;
    });
  }
