...
module.exports = function(app) {
  ...
  app.get('/.well-known/oauth2-client-configuration', function(req, res, next) {
    console.log('SUPPLYING THE CONFIG: ');
    
    var config = {
      client_name: "SSO Setup Client",
      redirect_uris: [ 'http://localhost:3000/connect-cb' ]
    }
  ...
  app.get('/connect', function(req, res, next) {
    ...
    url += '&state=foo'
    url += '&redirect_uri=' + encodeURIComponent('http://192.168.0.84:3000/connect-cb');
    url += '&scope=' + encodeURIComponent('openid create:clients');
    
    
    if(1) {
    var url = 'https://login0.myauth0.com/i/oauth2/authorize';
    url += '?client_id=' + encodeURIComponent('http://192.168.0.84:3000')
    url += '&response_type=code'
    url += '&state=foo'
